...
print('Start time: {}, run from {} to {}'.format(start_str, sys.argv[1], sys.argv[2]))
...
#            print("Device type:", cl.device_type.to_string(device.type))
    ...
    with open('run-{}-{}-{}.out'.format(sys.argv[1], sys.argv[2], step), 'a+') as writer:
        string = '{}, {}, {}, {}\n'.format(i, run[0], run[1], run[2])
        writer.write(string)
        print(string)
        
end_time = time()
end_str = strftime("%a, %d %b %Y %H:%M:%S", gmtime())
print('End time: {}, run from {} to {}'.format(end_str, sys.argv[1], sys.argv[2]))  
